                                 ...
                                 m_vmbSystem(VmbCPP::VmbSystem::GetInstance())
{
  ...
  // 10 == default QoS parameters with depth of 10
  trigger_subscription_ = this->create_subscription<cherry_interfaces::msg::Trigger>(
      "trigger",
      10, 
      std::bind(
          &Cic5000CameraHdr::trigger_callback,
          this,
          std::placeholders::_1));
  ...
  publisher_image_set_ = this->create_publisher<cherry_interfaces::msg::ImageSetHdr>("image_set", 10);
}
...
void Cic5000CameraHdr::inputs_callback(const cherry_interfaces::msg::Inputs &msg)
{
  ...
  {
    RCLCPP_DEBUG(this->get_logger(), "trigger ready = %d", triggerReady);
...
void Cic5000CameraHdr::trigger_callback(const cherry_interfaces::msg::Trigger &msg){
  try
  {
    // try to lock mutex to modify 'job_shared'
    if (trigger_mutex_.try_lock())
    {
     frame_id_ = msg.frame_id;
     // start execution thread
     std::thread trigger_thread(std::bind(&Cic5000CameraHdr::execute_trigger, this, msg));
     trigger_thread.detach();
     trigger_mutex_.unlock();
    }
    else
    {
        RCLCPP_ERROR(this->get_logger(), "Received trigger for frame %ld still capturing frame %ld.", msg.frame_id, frame_id_); 
    }
  }
  catch (const std::exception &e)
  {
    RCLCPP_ERROR(this->get_logger(), "Error triggering, err='%s'", e.what());
  }
}

void Cic5000CameraHdr::execute_trigger(const cherry_interfaces::msg::Trigger &msg){
  try
  {

    // try to lock mutex to modify 'job_shared'
    trigger_mutex_.lock();
    RCLCPP_ERROR(this->get_logger(), "Begin capturing frame %ld.", msg.frame_id); 
    // this image_set is used for image saving
    image_set_ = cherry_interfaces::msg::ImageSetHdr();
    image_set_.images = std::vector<cherry_interfaces::msg::ImageLayer>();
    image_set_.trigger = msg;

    // iamges are added to the image_set_.images when frames are called back
    // after the capture seqeunce queues them
    CaptureSequence(msg.frame_id);

    trigger_mutex_.unlock();

    publisher_image_set_->publish(image_set_);
    RCLCPP_DEBUG(this->get_logger(), "published image set");  
...
std::string Cic5000CameraHdr::uint64_to_string( uint64_t value ) {
...
void Cic5000CameraHdr::execute(const std::shared_ptr<GoalHandleAcquisition> goal_handle)
{
  ...
  try
  {
    // reinitialize the result object
    result_ = std::make_shared<cherry_interfaces::action::Acquisitionhdr::Result>();

    // // this image_set is used for image saving
    // image_set_ = cherry_interfaces::msg::ImageSetHdr();
    // image_set_.images = std::vector<cherry_interfaces::msg::ImageLayer>();
    // image_set_.frame_id = goal_handle->get_goal()->frame_id; 
    // image_set_.stamp = rclcpp::Clock(RCL_ROS_TIME).now();

    CaptureSequence(frame_id_);
...
void Cic5000CameraHdr::CaptureSequence(uint64_t frame_id)
{
  ...
  reset_request->frame_id = frame_id;
  ...
  if (VmbErrorSuccess == m_camera->GetFeatureByName("ExposureTimeAbs", pFeature))
  {
  }
  else
  {
    RCLCPP_ERROR(this->get_logger(), "Could not get exposure feature!");
  }

  if (VmbErrorSuccess == pFeature->SetValue(bot1_exposure_))
  {
  }
  else
  {
    RCLCPP_ERROR(this->get_logger(), "Could not set exposure feature!");
  }
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "Trigger bot 1 ready timeout");
  }
  else
  {
    RCLCPP_INFO(this->get_logger(), "Trigger ready for bot 1");
  }
  ...
  if (err == VmbErrorSuccess)
  {
    RCLCPP_INFO(this->get_logger(), "Triggered bottom 1 frame!");
  }
  else
  {
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Bot 1 notified");
  }
  ...
  if (VmbErrorSuccess == pFeature->SetValue(bot2_exposure_))
  {
  }
  else
  {
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "Trigger bot 2 ready timeout");
  }
  else
  {
    RCLCPP_INFO(this->get_logger(), "Trigger ready for bot 2");
  }
  ...
  if (err == VmbErrorSuccess)
  {
    RCLCPP_INFO(this->get_logger(), "Triggered bottom 2 frame!");
  }
  else
  {
    RCLCPP_ERROR(this->get_logger(), "Error triggering bottom 2 frame!");
  }
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Bot 2 notified");
  }
  ...
  // {
  //   RCLCPP_ERROR(this->get_logger(), "Trigger  ready timeout bot 3");
  // }
  // else
  // {
  //   RCLCPP_INFO(this->get_logger(), "Trigger ready for bot 3");
  // }
  ...
  // if (err == VmbErrorSuccess)
  // {
  //   RCLCPP_INFO(this->get_logger(), "Triggered bottom 3 frame!");
  // }
  // else
  // {
  //   RCLCPP_ERROR(this->get_logger(), "Error triggering bottom 3 frame!");
  // }
  ...
  // else
  // {
  //   RCLCPP_INFO(this->get_logger(), "bot 3 notified");
  // }
  ...
  // if (VmbErrorSuccess == pFeature->SetValue(top1_exposure_))
  // {
  // }
  // else
  // {
  ...
  // {
  //   RCLCPP_ERROR(this->get_logger(), "Trigger  ready timeou top 1t");
  // }
  // else
  // {
  //   RCLCPP_INFO(this->get_logger(), "Trigger ready for top 1 ");
  // }
  ...
  // if (err == VmbErrorSuccess)
  // {
  //   RCLCPP_INFO(this->get_logger(), "Triggered top 1 frame!");
  // }
  // else
  // {
  //   RCLCPP_ERROR(this->get_logger(), "Error triggering top 1 frame!");
  // }
  ...
  // else
  // {
  //   RCLCPP_INFO(this->get_logger(), "Top 1 notified");
  // }


 // ************************************************************ frame top 2 ****************************************************

  m_camera->QueueFrame(frame_top2_);
  ...
  if (VmbErrorSuccess == pFeature->SetValue(top2_exposure_))
  {
  }
  else
  {
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "Trigger  ready timeout top 2");
  }
  else
  {
    RCLCPP_INFO(this->get_logger(), "Trigger ready for top 2");
  }
  ...
  if (err == VmbErrorSuccess)
  {
    RCLCPP_INFO(this->get_logger(), "Triggered top 2 frame!");
  }
  else
  {
    RCLCPP_ERROR(this->get_logger(), "Error triggering top 2 frame!");
  }
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "Frame callback timeout 2");
  }
  else
  {
    RCLCPP_INFO(this->get_logger(), "Top 2 notified");
  }
...
//   {
//     RCLCPP_ERROR(this->get_logger(), "Trigger  ready timeout 3");
//   }
//   else
//   {
//     RCLCPP_INFO(this->get_logger(), "Trigger ready for top 3");
//   }
...
//   if (err == VmbErrorSuccess)
//   {
//     RCLCPP_INFO(this->get_logger(), "Triggered top 3frame!");
//   }
//   else
//   {
//     RCLCPP_ERROR(this->get_logger(), "Error triggering top 3frame!");
//   }
...
//   {
//     RCLCPP_ERROR(this->get_logger(), "Frame 6 callback timeout");
//   }
//   else
//   {
//     RCLCPP_INFO(this->get_logger(), "Top 3 notified");
//   }
...
// add a layer to the global _image_set_
void Cic5000CameraHdr::set_layer_data(const char* name, sensor_msgs::msg::Image image, uint64_t frame_id, int64_t count, int64_t mm){
  cherry_interfaces::msg::ImageLayer layer = cherry_interfaces::msg::ImageLayer();
  layer.name = name;
  layer.image = image;
  ...
  // push to global list
  image_set_.images.push_back(layer);
}
...
void Cic5000CameraHdr::frameCallback_top1(const VmbCPP::FramePtr frame)
{
  ...
  image_top.header = std_msgs::msg::Header();
  image_top.header.stamp = rclcpp::Clock(RCL_ROS_TIME).now();
  image_top.header.frame_id = uint64_to_string(frame_id_);
  ...
  // add to image set used for saving images
  set_layer_data("top1", image_top, frame_id_, stored_, stored_mm_ );

  // synchronize with the triggering thread
...
void Cic5000CameraHdr::frameCallback_top2(const VmbCPP::FramePtr frame)
{
  ...
  // add to image set used for saving images
  set_layer_data("top2", image_top, frame_id_, stored_, stored_mm_ );
...
void Cic5000CameraHdr::frameCallback_top3(const VmbCPP::FramePtr frame)
{
  ...
  // add to image set used for saving images
  set_layer_data("top3", image_top, frame_id_, stored_, stored_mm_ );
...
void Cic5000CameraHdr::frameCallback_bot1(const VmbCPP::FramePtr frame)
{
  ...
  image_bot.header = std_msgs::msg::Header();
  image_bot.header.stamp = rclcpp::Clock(RCL_ROS_TIME).now();
  image_bot.header.frame_id = uint64_to_string(frame_id_);
  ...
  // add to image set used for saving images
  set_layer_data("bot1", image_bot, frame_id_, stored_, stored_mm_ );
...
void Cic5000CameraHdr::frameCallback_bot2(const VmbCPP::FramePtr frame)
{
  ...
  // add to image set used for saving images
  set_layer_data("bot2", image_bot, frame_id_, stored_, stored_mm_ );
...
void Cic5000CameraHdr::frameCallback_bot3(const VmbCPP::FramePtr frame)
{
  ...
  // add to image set used for saving images
  set_layer_data("bot3", image_bot, frame_id_, stored_, stored_mm_ );
