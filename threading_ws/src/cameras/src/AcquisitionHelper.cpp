...
AcquisitionHelper::AcquisitionHelper(const char *cameraId) : m_vmbSystem(VmbCPP::VmbSystem::GetInstance())
{
...
{
    VmbErrorType err = m_camera->StartContinuousImageAcquisition(5, VmbCPP::IFrameObserverPtr(new FrameObserver(m_camera)));
    if (err != VmbErrorSuccess)
    {
