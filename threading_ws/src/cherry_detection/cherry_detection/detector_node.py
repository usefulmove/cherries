...
#sys.path.insert(1, '/home/user/cherry_ws/src/cherry_system/cherry_detection/cherry_detection/yolov7/')
...
class DetectorNode(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(Detectionhdr, '~/detect', self.detect_with_blob_callback)
        self.br = CvBridge()
        ...
        param_update = rclpy.Parameter('algorithm', rclpy.Parameter.Type.STRING, self.algorithm)
        ...
        self.publisher_detections_ = self.create_publisher(CherryArray, '~/detections', 10 )

        self.subcription_image_set_ = self.create_subscription(ImageSetHdr, 'image_set', self.image_set_callback, 10)
    ...
    def parameters_callback(self, params):
        ...
        for param in params:
            if param.name == 'simulate_detection':
                try:
                    self.get_logger().info('Set \'simulate_encoder\' to {}'.format(str(param.value)))
                    self.simulate_detection = param.value
                    # self.simulate_ticks()  # start the simulated upadte sequence

                except:
                    self.get_logger().error('Could not set \'simulate_encoder\' to {}'.format(param.value))
            ...
            if param.name == 'algorithm':
                try:
                    if param.value == 'fasterRCNN-Mask_ResNet50_V1':
                        ...
                        self.get_logger().error('Valid algorithm names: {}'.format(algorithms))
                        return SetParametersResult(successful=False)         
                    
                        
                except Exception as e:
                    self.get_logger().error('Could not set \'algorithm\' to {}'.format(param.value))
                ...
                try:
                    ...
                    if self.detector.pick_threshold < param.value:
                        self.get_logger().info('cannot set maybe value greater than pick_theshold'.format(str(param.value)))
                        return SetParametersResult(successful=False) 

                    self.get_logger().info('Set \'maybe_threshold\' to {}'.format(str(param.value)))
                    self.detector.maybe_threshold = param.value
                except:
                    self.get_logger().error('Could not set \'maybe_threshold\' to {}'.format(param.value))
                    return SetParametersResult(successful=False) 
            elif param.name == 'pick_threshold':
                try:
                    if self.detector.maybe_threshold > param.value:
                        self.get_logger().info('cannot set maybe value less than maybe_theshold'.format(str(param.value)))
                        return SetParametersResult(successful=False) 


                    self.get_logger().info('Set \'pick_threshold\' to {}'.format(str(param.value)))
                    self.detector.pick_threshold = param.value

                except:
                    self.get_logger().error('Could not set \'pick_threshold\' to {}'.format(param.value))
                    return SetParametersResult(successful=False) 
            else:
    ...
    def load_v3(self):
        ...
        # set up the classifier
        weight_path2 = os.path.join(package_share_directory, 'newlight-mask-12-15-2023.pt')
        weights2 = torch.load(weight_path2)
    ...
    def load_v4(self):
        ...
        # set up the classifier
        weight_path2 = os.path.join(package_share_directory,'newlights-nomask-12-15-2023.pt')
        weights2 = torch.load(weight_path2)
    ...
    def load_v6(self):
        ...
        weight_path_stem = os.path.join(package_share_directory,'stem_model_10_5_2024.pt')
    ...
    def load_v7(self):
        ...
        # set up the classifier
        weight_path2 = os.path.join(package_share_directory,'classification-202406031958-resnet50-adam-2.pt')
        weights2 = torch.load(weight_path2)
    ...
    def load_v8(self):
        ...
        self.detector = ai_detector_class_4(weights, weights_resnet50, weights_mobilenet, weights_densenet)
    ...
    def request_to_image_set(self, request : Detectionhdr.Request):
        ...
        image_set.trigger.stamp = self.get_clock().now().to_msg() # use the current time i guess
    ...
    def image_set_callback(self, image_set_hdr_msg : ImageSetHdr):
        ...
        self.get_logger().info('detect frame {} with encoder mm: {}'.format(
            image_set_hdr_msg.trigger.frame_id, 
            image_set_hdr_msg.trigger.encoder_mm) )
    ...
    def detect_with_blob_callback(self, request, response):
        ...
        self.get_logger().info('detect frame with encoder count:'.format(request.mm_bot1) )
