...
class DetectorNode(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(Detection, '~/detect', self.detect_with_blob_callback)
        self.br = CvBridge()
    ...
    def detect_with_blob_callback(self, request, response):
        ...
        # try:
        # get the color image
        response.encoder_count = request.count_bot1

        #self.get_logger().info('aquire image!' )
        # image_color = self.br.imgmsg_to_cv2(request.image, desired_encoding='passthrough')
        # self.get_logger().info('testtest: {}'.format(image_color))

        #self.get_logger().info('republish color image!' )

        #self.get_logger().info('do conversion!' )
        # detect cherries using a blob tool


        ## convert to PIL image
        # img_rgb = cv2.cvtColor(image_color, cv2.COLOR_RGB2BGR)

        dets, kp_im_procssed = self.detector.detect(request)
        print('********* dets **********')
        print(dets)
        print('********* **** **********')
        ...
        if self.simulate_detection == False:
            ...
            if (len(dets['labels']) > 0):
                ...
                #self.get_logger().info('dets: {}'.format( dets) )
                #self.get_logger().info('dets_scaled: {}'.format( dets) )
                ...
                for i in range(len(dets['labels'])):
                    ...
                    cherry.y = (xyxy[1] + xyxy[3]) / 2.0
                    cherry.type = (cls).to_bytes(1, 'big') # 1 is ok, 2 has a pit
                    response.cherries.append(cherry)
                #print(cherry)
        else:
            response.cherries = dets

        #     self.get_logger().info('Successful Detection!' )
        # except Exception as e: 
        #     self.get_logger().error('Detection error! error: {}'.format( e) )


        try:
            # publish the results
            cmsg = CherryArray()
            cmsg.encoder_count = request.encoder_count
            cmsg.cherries = response.cherries
