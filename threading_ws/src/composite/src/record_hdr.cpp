    ...
    : Node("main")
{
  ...
  detection_server_ = rclcpp_action::create_server<FindCherries>(
      this,
      "find_cherries",
      std::bind(&RecordHdr::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&RecordHdr::handle_cancel, this, std::placeholders::_1),
      std::bind(&RecordHdr::handle_accepted, this, std::placeholders::_1));
  ...
  detection_client_ = this->create_client<cherry_interfaces::srv::Detection>("detection_server/detect");
};

rclcpp_action::GoalResponse RecordHdr::handle_goal(
...
rclcpp_action::CancelResponse RecordHdr::handle_cancel(
...
void RecordHdr::handle_accepted(const std::shared_ptr<GoalHandleFindCherries> goal_handle)
{
  RCLCPP_INFO(this->get_logger(), "Starting find cherries thread");
  find_cherries_future_ = std::async(std::launch::async, &RecordHdr::execute, this, goal_handle);
};

void RecordHdr::execute(const std::shared_ptr<GoalHandleFindCherries> goal_handle)
{
  try
  {
    ...
    auto img = CombineImages(acquisition_result);

    auto detection_result = Detect(img, acquisition_result.mm_bot);
...
Acquisition::Result RecordHdr::AcquireImage(int frame_id)

{
  ...
  send_goal_options.goal_response_callback =
      std::bind(&RecordHdr::acquire_goal_response_callback, this, std::placeholders::_1);
  send_goal_options.feedback_callback =
      std::bind(&RecordHdr::acquire_feedback_callback, this, std::placeholders::_1, std::placeholders::_2);
  send_goal_options.result_callback =
      std::bind(&RecordHdr::acquire_result_callback, this, std::placeholders::_1);
...
void RecordHdr::SaveImages(Acquisition::Result result)
{
 std::string path = "/home/user/Pictures/hdr/latest/";
...
void RecordHdr::acquire_goal_response_callback(const GoalHandleAcquisition::SharedPtr &goal_handle){};
void RecordHdr::acquire_feedback_callback(
    GoalHandleAcquisition::SharedPtr,
    const std::shared_ptr<const Acquisition::Feedback> feedback){};
void RecordHdr::acquire_result_callback(const GoalHandleAcquisition::WrappedResult &result){};

bool RecordHdr::ResetPLC(int frame_id)
{
...
sensor_msgs::msg::Image RecordHdr::CombineImages(Acquisition::Result images)
{
...
cherry_interfaces::srv::Detection::Response::SharedPtr RecordHdr::Detect(sensor_msgs::msg::Image image, long frame_id)
{
  auto request =
      std::make_shared<cherry_interfaces::srv::Detection::Request>();

  request->encoder_count = frame_id;
  request->image = image;
