...
## Acceptance Criteria
- [ ] Train a model with `augmentation: true` that exceeds 93% validation accuracy.
- [ ] Evaluate at least 2 different hyperparameter configurations (learning rate, scheduler).
- [ ] (Optional) Compare ResNet50 with a lighter backbone (e.g., MobileNetV3) for inference speed speedup.
- [ ] Document final performance comparison table.
