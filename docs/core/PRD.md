...
## Goals
1. **Accuracy**: High recall for "Pit" class (minimum target to be defined) to ensure food safety.
2. **Efficiency**: High precision for "Clean" class to minimize the rejection of good fruit.
3. **Real-time Performance**: Maintain inference latency under 100ms per frame to match conveyor throughput.
